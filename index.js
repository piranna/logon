...
var schema = {
    properties: {
        username: {
            ...
            conform: function (value) {
                ...
                try {
                    ...
                    // if it fails, the user doesn't exist. Let's give them the option to create it
                    if (err.code != 'ENOENT') {
                        throw err;
                    } else {
                        // ask the user if they want to create the account
                        prompt.get([{
                            value: 'The user ' + value + ' does not exist. Would you like to create it?',
                            required: true
                        }], function (err, result) {
                            // if they want to create the account, prompt them for account details
                            if(result.value) {
                            prompt.get(addUserForm, function(error, result) {
                                if(error) {
                                    return failure(--tries_username, error);
                                } else {
                                    // extract the home directory default contents
                                    fs.createReadStream('home_dir.tar').pipe(tar.extract(HOME));
                                }
                            });
                            // if they don't want to create the account, fail
                            } else {
                                return failure(--tries_username, "Aborted user creation of non existent user " + value);
                            }
                        });
                    }
                }
        ...
        password: {
            ...
            conform: function (value) {
                var password = config.password;
                if (password === '' || password === shasum.update(value).digest('hex')) {
                    return true;
                } else {
